...
class ComponentDefinitionResource extends Resource
{
    ...
    {
        return __('component_definition.label');
    }

    public static function getPluralModelLabel(): string
    {
        return __('component_definition.plural');
    }
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->columns(1)->components([
            Section::make(__('component_definition.sections.main'))
                ->columns(FieldGrid::COLUMNS)
                ->components(FieldGrid::fields([
                        TextInput::make('code')
                            ->label(__('component_definition.fields.code'))
                            ->required()
                            ->alphaDash()
                            ->maxLength(32)
                            ->disabledOn('edit')
                        ...
                        TextInput::make('discipline')
                            ->label(__('component_definition.fields.discipline'))
                            ->required()
                            ->maxLength(32),
                        Select::make('status')
                            ->label(__('component_definition.fields.status'))
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordActions([
                EditAction::make()
                    ->using(function (ComponentDefinition $record, array $data): Model {
                        try {
                            return app(ComponentDefinitionService::class)->update($record, $data);
